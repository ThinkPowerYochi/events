...
$(function () {
	...
	// });
	getMotoTaxs();
	function getMotoTaxs() {
		$.ajax({
			type: "GET",
			url:"./Content/js/json.json",
		...
		function insertInfo(data) {
			var box = $('.searchbox_info .searchbox');
			var taxs = data.taxs;
			// box.html('');
			for(var i=0; i<taxs.length; i++) {
				box.append('<a href="'+ taxs[i].href +'">'+ taxs[i].title +'</a>');
				$(".searchbox a").slice(0,12).show();
			}
		}
	}
	$(function () {
		$("#loadMore").on('click', function (e) {
			e.preventDefault();
			$(".searchbox a:hidden").slice(0,12).slideDown();
			if ($(".searchbox a:hidden").length == 0) {
	...
	$('.landing__title').on('click', function () {
		$(this).parent().find('.landing__content').toggleClass('active');
	});
